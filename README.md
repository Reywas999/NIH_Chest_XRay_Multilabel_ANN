...
This personal project was just a proof of concept to get a working model that outputs likelihood values for each of the 14 potential outcome labels, given an input chest x-ray image (among other inputs). The f-score, precision, and accuracy are abysmal. I've also attempted to create an ensemble model by first using a CNN for feature extraction, and then training both a KNN and Random Forest classifier on these features, but these both returned only slightly better results.
